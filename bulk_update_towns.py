#!/usr/bin/env python3
"""Retirement gate for an obsolete, unsafe town-page mutation utility."""
...
# All town routes are managed elsewhere; the retired entry point owns none.
MANAGED_TOWN_RISK_SLUGS = frozenset()
MANAGED_NOINDEX_SLUGS = frozenset()
managed_slugs = frozenset({"*"})
REPLACEMENT_CHECKS = (
    "python3 tools/check_technical_seo.py",
    "python3 tools/check_english_fair_housing.py",
    "python3 scripts/remediate_indexable_towns.py --check",
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "RETIRED: bulk_update_towns.py does not modify files; run the read-only "
