...
- **Generators** (repo root): `town_data.py` (COUNTY + commute; no medians by design), `bulk_update_towns.py`, `generate_new_landing_pages.py`, `generate_missing_towns.py`, `generate_blog.py`, `daily-blog.py`. `gen_realtor_pages.py` is retired from content generation and now only maintains the redirect-only `noindex` compatibility fallbacks from `vercel.json`; use `--check` to verify them without writing.
